    ...
    for (let i = 0; i < students.length; i++) {
      if (verifySchema(students[i])) {
        githubAccountChecker('github.com', students[i].github_account)
          ...
          .catch((err)=>{
            // console.log(err)
            throw new Error('This github account does not exist: ' + students[i].github_account)
          })
        ...
        gitlabAccountChecker('gitlab.com', students[i].gitlab_account)
          ...
          .catch((err)=>{
            // console.log(err)
            throw new Error('This gitlab account does not exist: ' + students[i].gitlab_account)
          })
      }
    }
  } else {
